...
happy(107) ➞ false
	...
	public static boolean happy(int num) {
		  ...
		  while (true) {
			int summedDigits = sumOfDigits(num);
			if (summedDigits == 1) {
				return true;
			} 
			else if (summedDigits==4) {
				return false;
			}
			else {
				num = summedDigits;
	  ...
	  private static int sumOfDigits(int num) {
		String[] splitDigits = String.valueOf(num).split("");
		int sum = 0;
		for (int i =0; i<splitDigits.length; i++) {
			sum = sum +Integer.parseInt(splitDigits[i])*Integer.parseInt(splitDigits[i]) ;
		}
		return sum;
	}
